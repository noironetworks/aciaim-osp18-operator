...
var _ = BeforeSuite(func() {
	...
	err = aciaimv1alpha1.AddToScheme(scheme.Scheme)
	Expect(err).NotTo(HaveOccurred())
	...
	// Register MariaDB CRDs (for mocking)
	err = mariadbv1.AddToScheme(scheme.Scheme)
	Expect(err).NotTo(HaveOccurred())

	// Register Neutron CRDs (for mocking)
	err = neutronv1.AddToScheme(scheme.Scheme)
	Expect(err).NotTo(HaveOccurred())

	// Register RabbitMQ CRDs (for mocking)
	...
	err = (&controller.CiscoAciAimReconciler{
	...
	namespace = fmt.Sprintf("test-ns-%s", RandStringBytes(5))
	ns := &corev1.Namespace{
